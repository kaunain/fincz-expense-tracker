    ...
    <div class="reports-page">
      ...
      <div class="stats-grid">
        <div class="m3-card stat-card success">
          ...
          <span class="stat-value income-text">+₹{{ currentMonthStats().income | number:'1.2-2' }}</span>
        </div>
        <div class="m3-card stat-card primary">
          <span class="stat-label">Expenses</span>
          <span class="stat-value expense-text">-₹{{ currentMonthStats().expenses | number:'1.2-2' }}</span>
        </div>
...
export class ReportsComponent {
  ...
  public currentMonthStats = computed(() => {
    ...
    return { income, expenses, balance: income - expenses, count: filtered.length, items: filtered };
  });
  ...
  public lastSixMonthsTrend = computed(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      this.expenses().forEach(e => {
        ...
        if (ed.getFullYear() === d.getFullYear() && ed.getMonth() === d.getMonth() && e.type !== 'income') {
          amount += e.amount;
        }
      });
  ...
  public exportCSV(): void {
    ...
    link.setAttribute('download', `fincz_transactions_${this.viewDate.getFullYear()}_${this.viewDate.getMonth() + 1}.csv`);
