...
@Component({
  ...
  template: `
    <div class="dashboard-page">
      ...
      <div class="month-nav">
        ...
        <button class="month-btn" (click)="nextMonth()" matRipple [disabled]="isCurrentMonth()">›</button>
      </div>
      ...
      <div class="hero-card">
        <div class="hero-metric">
          ...
          <span class="metric-value income">₹{{ monthSummary().monthlyIncome | number:'1.0-0' }}</span>
        </div>
        ...
        <div class="hero-metric">
          ...
          <span class="metric-value expense">₹{{ monthSummary().monthlySpent | number:'1.0-0' }}</span>
        </div>
      </div>
      ...
      <div class="m3-card chart-card" *ngIf="monthSummary().categoryBreakdown.length > 0">
        ...
        <div class="chart-layout">
          ...
          ></app-donut-chart>
          <div class="legend">
            <div *ngFor="let item of monthSummary().categoryBreakdown | slice:0:5" class="legend-item">
  ...
  styles: [`
    ...
    .metric-value.income { color: #86efac; }
    .metric-value.expense { color: #fca5a5; }
    .metric-value.balance { color: white; font-size: 1.5rem; }
    .metric-value.balance.negative { color: #fca5a5; }
    .hero-divider {
      width: 1px;
      height: 40px;
...
export class DashboardComponent {
  ...
  monthSummary = computed(() => {
    ...
    const categoryBreakdown = Array.from(categoryMap.entries()).map(([category, amount]) => ({
      category,
      amount,
      color: this.expenseService.financialSummary().categoryBreakdown.find(c => c.category === category)?.color || '#94a3b8',
  ...
  openAdd(type: TransactionType): void {
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result?.title && result?.amount) {
        ...
        this.snackBar.open(`${type === 'income' ? 'Income' : 'Expense'} added! 🎉`, 'OK', { duration: 2500 });
      }
    });
  }
}
